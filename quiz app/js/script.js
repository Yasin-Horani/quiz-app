...
function eventlisteners() {
  ...
  _playAgainBtn.addEventListener("click", restarQuiz);
}
  ...
  if (_options.querySelector(".selected")) {
    ...
    if (selectedAnswer.trim() == HTMLDecode(correctAnswer)) {
      ...
      _result.innerHTML = `<p><i class="fas fa-times"></i>
        Incorrect Answer! </p><p> <small><b>Correct Answer:</b> ${correctAnswer}</small>
        </p>`;
    }
    checkCount();
  } else {
    _result.innerHTML = `<p> <i class = "fas fa-check"></i>
    Select an Option!</p>`;
    _checkBtn.disabled = false;
  }
}

// to convert html entites into normal text of correct answer if there is any
